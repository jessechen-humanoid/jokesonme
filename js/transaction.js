...
(async function () {
  ...
  // Fund checkbox handler
  const fundCheckbox = document.getElementById('tx-paid-by-fund');
  if (fundCheckbox) {
    fundCheckbox.addEventListener('change', toggleAllocationFields);
  }
...
function toggleAllocationFields() {
  ...
  const fundGroup = document.getElementById('tx-fund-group');
  const fundCheckbox = document.getElementById('tx-paid-by-fund');
  ...
  if (currentTxType === 'income') {
    ...
    if (fundGroup) fundGroup.style.display = 'block';
    const fundPaid = fundCheckbox && fundCheckbox.checked;
    memberGroup.style.display = fundPaid ? 'none' : 'block';
    allocationGroup.style.display = fundPaid ? 'none' : 'block';
    if (allocationLabel) allocationLabel.textContent = '分擔成員';
  }
}

// 改這裡必須同步改 gas/Code.gs 的 FUND_SHOW_NAME
...
async function onShowSelected(showName) {
  ...
  // 共同基金支出 → 預設支出模式 + 自動勾選「由共同基金支付」
  if (showName === FUND_SHOW_NAME) {
    currentTxType = 'expense';
    document.querySelectorAll('#tx-type-toggle .toggle-btn').forEach(b => {
      b.classList.toggle('active', b.dataset.type === 'expense');
    });
    updateCategorySelect('tx-category', 'expense', '');
    const fundCheckbox = document.getElementById('tx-paid-by-fund');
    if (fundCheckbox) fundCheckbox.checked = true;
    toggleAllocationFields();
  }
    ...
    <div class="card">
      ...
      <div class="table-wrapper">
        <table>
          ...
          <tbody>
            ${transactions.map(t => `
              <tr class="${!t.settled && t.advancedBy ? 'unsettled' : ''} ${t.autoGenerated ? 'auto-row' : ''}" data-id="${t.id}">
                <td>
                  ${escapeHtml(t.category || '')}
                  ${t.autoGenerated ? '<span class="tx-tag tx-tag-auto">自動</span>' : ''}
                  ${t.paidByFund ? '<span class="tx-tag tx-tag-fund">基金</span>' : ''}
                </td>
                ...
                <td>
                  ${t.autoGenerated
                    ? '<span style="color:var(--text-secondary);font-size:12px;">系統</span>'
                    : `<div class="tx-actions">
                        <button class="btn-actions" data-id="${t.id}">⋯</button>
                        <div class="action-menu" id="menu-${t.id}">
                          <button class="action-menu-item btn-edit" data-id="${t.id}" data-category="${escapeAttr(t.category || '')}" data-notes="${escapeAttr(t.notes || '')}" data-amount="${t.amount}" data-advanced-by="${escapeAttr(t.advancedBy || '')}" data-excluded-members="${escapeAttr(t.excludedMembers || '')}" data-date="${t.date || ''}" data-recorded-by="${escapeAttr(t.recordedBy || '')}" data-paid-by-fund="${t.paidByFund ? '1' : '0'}">編輯</button>
...
function startEdit(id, category, notes, amount, advancedBy, excludedMembers, date, recordedBy, paidByFund) {
  editingId = id;
  ...
  // Set fund checkbox
  const fundCheckbox = document.getElementById('tx-paid-by-fund');
  if (fundCheckbox) fundCheckbox.checked = !!paidByFund && !isIncome;
...
function cancelEdit() {
  ...
  // Reset fund checkbox (but preserve when the selected show is 共同基金支出)
  const fundCheckbox = document.getElementById('tx-paid-by-fund');
  if (fundCheckbox) fundCheckbox.checked = currentShow === FUND_SHOW_NAME;
...
async function submitTransaction() {
  ...
  const fundCheckbox = document.getElementById('tx-paid-by-fund');
  const paidByFund = !isIncome && fundCheckbox && fundCheckbox.checked;
  const advancedBy = (isIncome || paidByFund) ? '' : getMemberValue('tx-member');
  const excludedMembers = paidByFund ? '' : getExcludedMembers('tx-allocation-container');
