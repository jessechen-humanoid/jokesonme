...
// 1=專案名稱, 2=分類, 3=備註, 4=金額, 5=墊款人, 6=排除成員, 7=結清狀態, 8=日期, 9=登記人, 10=由共同基金支付, 11=系統自動, 12=ID(UUID)
const TX_COL = {
  SHOW: 1, CATEGORY: 2, NOTES: 3, AMOUNT: 4, ADVANCED_BY: 5,
  EXCLUDED: 6, SETTLED: 7, DATE: 8, RECORDER: 9, PAID_BY_FUND: 10, AUTO_GENERATED: 11, ID: 12,
};
const TX_HEADERS = ['專案名稱', '分類', '備註', '金額', '墊款人', '排除成員', '結清狀態', '日期', '登記人', '由共同基金支付', '系統自動', 'ID'];
...
// 會寫入試算表的 action，進入前需取得 script lock 以序列化並發寫入
const WRITE_ACTIONS = [
  'addShow', 'addTransaction', 'updateTransaction', 'deleteTransaction',
  'initChecklist', 'updateChecklistItem', 'addChecklistItem',
  'batchImportTransactions', 'addSettlement', 'addAdvanceReimbursement',
  'updateForecast',
];
...
function handleRequest(e) {
  ...
  try {
    if (action === 'verifyPassword') {
      ...
      // 寫入類 action 取得 script lock，序列化並發寫入；讀取類不上鎖
      const lock = WRITE_ACTIONS.indexOf(action) !== -1 ? LockService.getScriptLock() : null;
      if (lock && !lock.tryLock(20000)) {
        result = { success: false, error: '系統忙碌中，請稍後再試' };
      } else {
      try {
      switch (action) {
        case 'getShows':
...
function addTransactionInternal(payload, isInternal) {
  ...
  sheet.appendRow([
    ...
    settled, date, recordedBy, paidByFund, autoGenerated, Utilities.getUuid(),
  ]);
...
// 以 UUID 找到交易列號；找不到回 -1。這是修正「列號位移導致改到／刪到別筆」的核心。
function findTxRowByUuid_(sheet, uuid) {
  ...
  for (let i = 0; i < ids.length; i++) {
    if ((ids[i][0] || '').toString() === id) return i + 2;
  }
  return -1;
}
...
function updateTransactionInternal(payload, isInternal) {
  const uuid = (payload.id || '').toString();
  if (!uuid) {
    return { success: false, error: '無效的交易 ID' };
  }
  const ss = getSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAMES.TRANSACTIONS);
  const rowId = findTxRowByUuid_(sheet, uuid);
  if (rowId < 2) {
    return { success: false, error: '找不到該筆交易（可能已被刪除）' };
  }
...
// ============================================================
// One-time（編輯器執行）：為既有交易列補 UUID
// 由 Jesse 於 Apps Script 編輯器選此函式 Run 一次即可；冪等、可重跑，不進 API 路由。
// ============================================================
function migrateBackfillTxIds() {
  const ss = getSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAMES.TRANSACTIONS);
  if (!sheet) return { success: false, error: '找不到收支紀錄工作表' };

  // 確保 ID 欄表頭存在
  const headerCell = sheet.getRange(1, TX_COL.ID);
  if (!(headerCell.getValue() || '').toString().trim()) {
    headerCell.setValue('ID').setFontWeight('bold');
  }

  const lastRow = sheet.getLastRow();
  if (lastRow <= 1) return { success: true, backfilled: 0 };

  const idRange = sheet.getRange(2, TX_COL.ID, lastRow - 1, 1);
  const ids = idRange.getValues();
  let backfilled = 0;
  for (let i = 0; i < ids.length; i++) {
    if (!(ids[i][0] || '').toString().trim()) {
...
function recalcTaxReserveForShow(showName) {
  ...
  Object.keys(groups).forEach(key => {
    ...
    sheet.appendRow([
      ...
      '', key, '已結清', dateStr, SYSTEM_RECORDER, false, true, Utilities.getUuid(),
    ]);
  });
}
...
function batchImportTransactions(payload) {
  ...
  // Headers: 專案名稱, 分類, 備註, 金額, 墊款人, 排除成員, 結清狀態, 日期, 登記人, 由共同基金支付, 系統自動, ID
  ...
  sheet.getRange(sheet.getLastRow() + 1, 1, rows.length, TX_NUM_COLS).setValues(rows);
